...
csharp_library_attr.update(_COMMON_ATTRS)
csharp_library = rule(
    implementation = _lib_impl,
    attrs = csharp_library_attr,
    outputs = {'lib': '%{name}.dll', 'doc': '%{name}.xml', 'mdb': '%{name}.dll.mdb'}
)

csharp_binary_attr = {'runargs': attr.string_list(), '_target_type': attr.string(default='bin')}
csharp_binary_attr.update(_COMMON_ATTRS)
csharp_binary = rule(
    implementation = _bin_impl,
    attrs = csharp_binary_attr,
