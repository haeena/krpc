...
def _sdist_impl(ctx):
    ...
    staging_inputs = []
    for input in inputs:
        staging_path = ctx.configuration.genfiles_dir.path + '/' + staging_dir + '/' + _apply_path_map(path_map, input.short_path)
...
def _script_impl(ctx):
    script_setup = ctx.actions.declare_file('{}/py_script-setup-{}'.format(ctx.configuration.genfiles_dir.path, ctx.attr.script))
    script_env = ctx.actions.declare_file('{}/py_script-env-{}'.format(ctx.configuration.genfiles_dir.path, ctx.attr.script))
